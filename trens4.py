...
velocidade = {'verde': 0.002, 'roxo': 0.003, 'vermelho': 0.005, 'azul': 0.001}
...
lado = 15
posicaox = {'verde': 83, 'roxo': 303, 'vermelho': 523, 'azul': 83}
posicaoy = {'verde': 43, 'roxo': 43, 'vermelho': 43, 'azul': 253}
  ...
  while running[0]:
    for event in pygame.event.get():
      ...
      if event.type == pygame.KEYDOWN:
        if event.key == pygame.K_t and velocidade['verde'] <= 0.0095:
          velocidade['verde'] += 0.0005
        if event.key == pygame.K_g and velocidade['verde'] >= 0.0005:
          velocidade['verde'] -= 0.0005
        if event.key == pygame.K_y and velocidade['roxo'] <= 0.0095:
          velocidade['roxo'] += 0.0005
        if event.key == pygame.K_h and velocidade['roxo'] >= 0.0005:
          velocidade['roxo'] -= 0.0005
        if event.key == pygame.K_u and velocidade['vermelho'] <= 0.0095:
          velocidade['vermelho'] += 0.0005
        if event.key == pygame.K_j and velocidade['vermelho'] >= 0.0005:
          velocidade['vermelho'] -= 0.0005  
        if event.key == pygame.K_i and velocidade['azul'] <= 0.0095:
          velocidade['azul'] += 0.0005  
        if event.key == pygame.K_k and velocidade['azul'] >= 0.0005:
          velocidade['azul'] -= 0.0005
...
# Threads   
thread_controle = Thread(target = controle)
thread_controle.start()

# Movimentos
thread_verde = Thread(target = movimento_verde)
thread_verde.start()
thread_roxo = Thread(target = movimento_roxo)
thread_roxo.start()
thread_vermelho = Thread(target = movimento_vermelho)
thread_vermelho.start()
thread_azul = Thread(target = movimento_azul)
thread_azul.start()
...
while running[0]:
  ...
  pygame.draw.rect(screen, 'mediumseagreen', [290, 660 - 20000*velocidade['verde'], 30, 10])
  pygame.draw.rect(screen, 'white', [367, 460, 10, 210])
  pygame.draw.rect(screen, 'purple', [357, 660 - 20000*velocidade['roxo'], 30, 10])
  pygame.draw.rect(screen, 'yellow', [433, 460, 10, 210])
  pygame.draw.rect(screen, 'red', [423, 660 - 20000*velocidade['vermelho'], 30, 10])
  pygame.draw.rect(screen, 'royalblue', [500, 460, 10, 210])
  pygame.draw.rect(screen, 'blue', [490, 660 - 20000*velocidade['azul'], 30, 10])
   
  pygame.display.update()

# Exibicao
#thread_exibicao = Thread(target = exibicao)
...
thread_verde.join()
thread_roxo.join()
thread_vermelho.join()
thread_azul.join()
#thread_exibicao.join()
